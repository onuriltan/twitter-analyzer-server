...
public class BaseTwitterStream {
  ...
  ApplicationContext context =
      SpringApplicationContext
          .getApplicationContext(); // get application context to inject beans to non managed spring
                                    // class(this)

  private TweetAnalyzer tweetAnalyzer = (TweetAnalyzer) context.getBean("tweetAnalyzer");
  private GeocodeGenerator geocodeGenerator =
      (GeocodeGenerator) context.getBean("geocodeGenerator");
  ...
  private WebSocketSessionHandler webSocketSessionHandler =
      (WebSocketSessionHandler) context.getBean("webSocketSessionHandler");
  private TwitterStream twitterStream = twitterStream();
  private TwitterRateLimitHandler twitterRateLimitHandler =
      (TwitterRateLimitHandler) context.getBean("twitterRateLimitHandler");
      ...
      StatusListener listener =
          new StatusListener() {
            public void onStatus(Status status) {
              ...
              if (status.getGeoLocation() != null) {
                ...
                GeocodeResponse geocodeResponse =
                    geocodeGenerator.getLatLong(status.getUser().getLocation());
            ...
            public void onException(Exception ex) {
              ...
              tweetAnalyzer.sendException(
                  sessionId, String.valueOf(twitterException.getStatusCode()));
              twitterStream.clearListeners();
              twitterStream.shutdown();
              twitterRateLimitHandler.setWhenRateLimitOccured(Calendar.getInstance().getTime());
            }
          };
      if (twitterRateLimitHandler.getWhenRateLimitOccured() != null
          && new Date().getTime() - twitterRateLimitHandler.getWhenRateLimitOccured().getTime()
              >= 15 * 60 * 1000) {
  ...
  public TwitterStream twitterStream() {
    TwitterStreamFactory twitterStreamFactory =
        new TwitterStreamFactory(twitterConfig.getConfiguration());
